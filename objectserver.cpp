...
ObjectServer::ObjectServer() {
    m_serverObj = (lwm2m_object_t *)lwm2m_malloc(sizeof(lwm2m_object_t));
    memset(m_serverObj, 0, sizeof(lwm2m_object_t));
}

void ObjectServer::free_server_object(lwm2m_object_t* object) {
    while (object->instanceList != nullptr)
    {
        auto * serverInstance = object->instanceList;
...
lwm2m_object_t *ObjectServer::get_server_object() {

    if (nullptr != m_serverObj)
    {
        ...
        m_serverObj->instanceList = LWM2M_LIST_ADD(m_serverObj->instanceList, serverInstance);

        m_serverObj->readFunc = prv_server_read;
        m_serverObj->writeFunc = prv_server_write;
        m_serverObj->createFunc = prv_server_create;
        m_serverObj->deleteFunc = prv_server_delete;
        m_serverObj->executeFunc = prv_server_execute;
        m_serverObj->discoverFunc = prv_server_discover;
    }

    return m_serverObj;
}

uint8_t ObjectServer::prv_server_read(uint16_t instanceId, int *numDataP, lwm2m_data_t **dataArrayP, lwm2m_object_t *objectP) {
...
uint8_t ObjectServer::prv_get_value(lwm2m_data_t * dataP,
                                    server_instance_t * targetP) {
    switch (dataP->id)
    {
...
uint8_t ObjectServer::prv_server_write(uint16_t instanceId, int numData, lwm2m_data_t *dataArray, lwm2m_object_t *objectP) {
    server_instance_t * targetP;
    int i;
    uint8_t result;
...
uint8_t ObjectServer::prv_set_int_value(lwm2m_data_t *dataArray, uint32_t *data) {
    uint8_t result;
    int64_t value;
...
uint8_t ObjectServer::prv_server_create(uint16_t instanceId, int numData, lwm2m_data_t *dataArray, lwm2m_object_t *objectP) {
    server_instance_t * serverInstance;
    uint8_t result;

    serverInstance = (server_instance_t *)lwm2m_malloc(sizeof(server_instance_t));
    if (nullptr == serverInstance) return COAP_500_INTERNAL_SERVER_ERROR;
    memset(serverInstance, 0, sizeof(server_instance_t));
...
uint8_t ObjectServer::prv_server_delete(uint16_t id, lwm2m_object_t *objectP) {
    server_instance_t * serverInstance;

    objectP->instanceList = lwm2m_list_remove(objectP->instanceList, id, (lwm2m_list_t **)&serverInstance);
    if (nullptr == serverInstance) return COAP_404_NOT_FOUND;
...
uint8_t ObjectServer::prv_server_execute(uint16_t instanceId, uint16_t resourceId, uint8_t *buffer, int length,
...
uint8_t ObjectServer::prv_server_discover(uint16_t instanceId, int *numDataP, lwm2m_data_t **dataArrayP,
                                          lwm2m_object_t *objectP) {
    uint8_t result;
    int i;
